...
$title=addslashes('airtime');
require_once('include/user_header.php');
require_once('include/user_navbar.php');
require_once('include/user_sidebar.php');
/*
if(isset($_REQUEST['deno']) && isset($_REQUEST['net']) && isset($_REQUEST['phone']) && isset($_REQUEST['user_id'])){
include("../app/phperr.php");

} */
//$shoGod=mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM God WHERE uid='1'"));
      ...
      <div class="page-header page-header-light">
        <div class="page-header-content header-elements-md-inline">
          <div class="page-title d-flex">
            <h4><i class="icon-cart-add mr-2"></i> <span class="font-weight-semibold">Order For Airtime</span></h4>
          </div>
        </div>
...
<?php }?>
     ...
     <div class="card border-top-3 border-top-danger rounded-left-0">
        <div class="card-header header-elements-inline">
          <h6 class="card-title font-weight-semibold">Get instant top up</h6>
        ...
        <div class="card-body">
          ...
          <form id="pur" action="../app/calculator/norder" method="post">

<table><tr><td><h5><div id='nt' style='text-transform:uppercase'></div>
  </h5></td><td><h5>Airtime vtu</h5></td><td style="padding-left:20px;">
</td></tr></table>
...
<br/>
    ...
    <option value="select">Click To Select network</option>
...
$ident_serviceID=strtoupper($ident_row['identifier_serviceID']);
  ...
  <input type="hidden" name="page" id="page" value="airtime"> 
...
<input value="<?php echo clean($encrypted_value);?>" name="temp_ran" id="temp_ran" type="hidden"  />
 <input type="hidden" name="token" id="token" value="<?php echo clean($token);?>"> 
              ...
              <div class="form-group row">
               <div class="col-lg-6">
                  <input type="text" class="form-control" name="deno" id="deno" value="100" required />
           
                  </div>
                </div>

            <p>Amount To Pay</p>
<div id='percent'>waiting for network...</div>
              </div>              
